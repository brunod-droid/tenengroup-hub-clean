...
export function ReportingNav() {
  const items = [
    ["Dashboard", "/yves-rocher-reporting"],
  ...
  return (
    <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 22 }}>
      {items.map(([label, href]) => (
        <Link key={href} href={href} style={{ background: "#0f172a", color: "#fff", borderRadius: 12, padding: "10px 14px", textDecoration: "none", fontWeight: 700 }}>
          {label}
        </Link>
      ))}
    </div>
