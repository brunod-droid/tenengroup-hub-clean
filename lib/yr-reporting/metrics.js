...
function buildAgentDrilldown(tickets = [], agentMetrics = []) {
  ...
  function ensure(name) {
    ...
    if (!map[group]) {
      map[group] = { name: group, role: agentRole(group), assignedTickets: 0, messagesSent: 0, csatTotal: 0, csatCount: 0, frtTotal: 0, frtCount: 0 };
    }
    return map[group];
  }
...
function uniquePaidOrders(orders = []) {
  const seen = new Set();
  const paid = [];
  ...
  orders.forEach((row, index) => {
    ...
    const id = row.Id || row.ID || row.id || row.Name || row.name || `row-${index}`;
    const key = String(id || "").trim() || `row-${index}`;
...
export function calculateWeeklyMetrics(report, settings) {
  ...
  const paidOrders = uniquePaidOrders(orders);
  const ordersCount = paidOrders.length || sumBy(orders, [/orders/, /order count/, /total orders/]) || orders.length;
  ...
  return {
    ...
    backlog,
    ordersCount,
    unpaidOrRemovedOrders: orders.length ? Math.max(orders.length - ordersCount, 0) : 0,
