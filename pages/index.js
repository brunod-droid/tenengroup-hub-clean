...
const MENU = ["Home","Training","Brands","Cases","Policies","Events","CRM","Logistics","Yves Rocher","Social Policy","QA Team","OCy","AI Agents","Q&A"];
...
const CASES = [
  { id:"presales", name:"Pre-sales", short:"Questions before purchase.", full:["Product questions","Material questions","Personalization possibilities","Maximum characters or special requests","Delivery promise before purchase","Countries served"] },
...
function assistantAnswer(input) {
  ...
  if (q.includes("training") || q.includes("orientation")) return { title:"Training guidance", body:"Open the Training page. It is designed as a full-screen 20-minute presentation for new company employees.", tags:["Training"] };
  ...
  if (q.includes("oak") && (q.includes("negative") || q.includes("comment") || q.includes("criticism"))) return { title:"Oak & Luna social criticism", body:"Use approved neutral public wording, move to DM, do not explain publicly, and hide the negative comment after the approved reply.", tags:["Social","Oak & Luna"] };
  ...
  if (q.includes("late") || q.includes("delay")) return { title:"Late order guidance", body:"Check ETA first. Under 3 business days late: apologize and give ETA. Over 3 business days late: compensation may apply.", tags:["WISMO","Late"] };
  ...
  return <div style={{ scrollBehavior:"smooth" }}>
    ...
    <section id="intro" style={{ minHeight:"100vh", padding:70, display:"flex", alignItems:"center" }}>
      <div>
        ...
        <div style={{ marginTop:34 }}><Pill>Team</Pill><Pill>Mission</Pill><Pill>Channels</Pill><Pill>KPIs</Pill><Pill>Customer Journey</Pill><Pill>Trustpilot</Pill></div>
      </div>
    </section>
    ...
    <section id="org" style={{ minHeight:"100vh", padding:70, background:"radial-gradient(circle at 20% 25%, rgba(37,99,235,.35), transparent 18%), radial-gradient(circle at 70% 45%, rgba(34,197,94,.28), transparent 18%), linear-gradient(135deg,#020617,#0f172a)", color:"#fff" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1.2fr 1fr", gap:26, marginTop:40 }}>
        <div>
          <div style={{ background:"rgba(255,255,255,.08)", borderRadius:24, padding:26, border:"1px solid rgba(255,255,255,.15)" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:18, marginTop:22 }}>
            <div style={{ background:"rgba(255,255,255,.08)", borderRadius:22, padding:22 }}>
              ...
              <div style={{ marginTop:14, lineHeight:1.9, color:"#e5e7eb" }}>Bruno Dreyfus — CS Director<br/>Laurence Darmon — QA Team Leader<br/>Orly Ancel — Order Cycle Team Leader<br/>Adi Zaken — Team Leader<br/>Thijs Vandenbroucke — Team Leader<br/>Neva Zec — Success<br/>CSR teams</div>
            </div>
            <div style={{ background:"rgba(255,255,255,.08)", borderRadius:22, padding:22 }}>
        ...
        <div style={{ background:"rgba(255,255,255,.08)", borderRadius:24, padding:26 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginTop:22 }}>
            {COUNTRIES.map((c) => <div key={c} style={{ background:"rgba(255,255,255,.1)", borderRadius:12, padding:12 }}>{c}</div>)}
          </div>
        </div>
      </div>
    </section>
    ...
    <section id="mission" style={{ minHeight:"100vh", padding:70 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:28, marginTop:36 }}>
        <Box><div style={{ fontSize:32, fontWeight:900 }}>Mission</div><Bullets items={["Optimize customer experience through tone of voice, policies and service quality.","Maximize lifetime value and reduce customer churn.","Identify and implement new channels to enhance satisfaction.","Coordinate and bring insights to Shipping, Factory and Brands."]} /></Box>
        <Box><div style={{ fontSize:32, fontWeight:900 }}>Where customers reach us</div><Bullets items={["Webform and emails","Social media: Facebook and Instagram","Trustpilot","Other public platforms: Reddit, BBB, complaint websites"]} /></Box>
      </div>
      <Box><div style={{ fontSize:32, fontWeight:900 }}>Our KPIs</div><div style={{ display:"grid", gridTemplateColumns:"repeat(4, 1fr)", gap:16, marginTop:20 }}>{[["CSAT","Customer satisfaction after an interaction."],["SLA","How fast we respond and respect timing commitments."],["NPS","Customer loyalty and likelihood to recommend."],["Order Cost","Efficiency and cost of resolving customer issues."]].map(([title, text]) => <div key={title} style={{ background:"#f8fafc", borderRadius:18, padding:18 }}><div style={{ fontSize:26, fontWeight:900, color:"#2563eb" }}>{title}</div><div style={{ marginTop:10, color:"#4b5563", lineHeight:1.6 }}>{text}</div></div>)}</div></Box>
    </section>
    ...
    <section id="wheel" style={{ minHeight:"100vh", padding:70 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(5, 1fr)", gap:14, marginTop:36 }}>{[["1","Questions","Products, shipping, warranty, special requests, payment, technical issues, coupons."],["2","Change Order","Address, item, inscription, shipping method."],["3","WISMO","Late supplier, late, on time, lost, DNR, returned to sender."],["4","Item Received","Damaged, not satisfied, production mistake, customer mistake."],["5","Other","Account issues, collaboration, spam."]].map(([num,title,text]) => <div key={title} style={{ border:"1px solid #e5e7eb", borderRadius:22, padding:20, background:"#fff" }}><div style={{ width:40, height:40, borderRadius:"50%", background:"#2563eb", color:"#fff", display:"flex", alignItems:"center", justifyContent:"center", fontWeight:900 }}>{num}</div><div style={{ fontSize:22, fontWeight:900, marginTop:14 }}>{title}</div><div style={{ color:"#4b5563", lineHeight:1.6, marginTop:10 }}>{text}</div></div>)}</div>
      <Box><div style={{ fontSize:30, fontWeight:900 }}>Handled by</div><div style={{ marginTop:14 }}><Pill>CS — Customer Service</Pill><Pill>AI — Notch & Internal AI</Pill><Pill>OCy — Order Cycle</Pill><Pill>QA — Quality & Escalations</Pill></div></Box>
  ...
  return <div style={{ display:"flex", minHeight:"100vh", background:"#f5f7fb", fontFamily:"Arial, sans-serif" }}>
    ...
    <main style={{ flex:1, padding:24 }}>
      ...
      {Object.keys(pageData).includes(page) && <>
        ...
        {pageData[page].map((x) => <ExpandableCard key={x.id} title={x.name || x.title} shortText={x.short} bullets={x.full} extraTitle={x.tone ? "Tone of voice" : null} extraItems={x.tone || null} wording={x.wording || null} />)}
      </>}
      ...
      {page === "Events" && <>
        ...
        {EVENTS.map((e) => <Box key={e.id}><div style={{ fontSize:28, fontWeight:800 }}>{e.name}</div><div style={{ marginTop:10, color:"#4b5563", lineHeight:1.7 }}>{e.short}</div><div style={{ marginTop:12, color:"#374151", lineHeight:1.7 }}>{e.intro}</div>{e.sections.map((section) => <div key={section.title} style={{ marginTop:20 }}><div style={{ fontWeight:800, marginBottom:8 }}>{section.title}</div><Bullets items={section.items} /></div>)}<div style={{ marginTop:18, fontWeight:800 }}>Suggested wording</div><div style={{ marginTop:8, fontStyle:"italic", color:"#374151", lineHeight:1.7 }}>{e.wording}</div></Box>)}
      </>}
