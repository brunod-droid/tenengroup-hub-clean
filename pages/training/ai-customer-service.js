...
const slides = [
  {
    ...
    bullets: ["GDPR matters", "Use approved tools", "Practical demos", "One useful idea for tomorrow"],
  },
  {
    type: "quiz",
  ...
  },
  {
    ...
    subtitle: "Agents are useful when a task repeats and needs information.",
    bullets: [
  ...
  },
  {
    ...
    subtitle: "Vote by hand. We will prioritize the most relevant demos.",
    cards: [
  ...
  },
  {
    ...
    cards: [
      ...
      ["Project", "Shared workspace", "Files + chats + team context"],
  ...
  },
  {
    ...
    subtitle: "It can understand both the facts and the emotional priority.",
    leftTitle: "Customer message",
    left: "Sentimental locket. Unexpected delay. Customer asks us to push production and get a clear new arrival date.",
    rightTitle: "AI-assisted answer",
    right: "Empathy, ownership, reassurance, clear next step, professional tone.",
    bullets: ["Empathy", "Clarity", "Ownership", "Consistency"],
  },
  {
    type: "rewrite",
    title: "One message. Five useful versions.",
    subtitle: "Same input, different business need.",
    original: "where is my order i wait since 2 weeks no answer this is unacceptable",
    versions: [
      ["Professional", "I am sorry for the delay and I understand your frustration. I will check your order status and get back to you with a clear update as soon as possible."],
      ["Empathetic", "I completely understand how disappointing it is to wait without a clear update. Let me look into this right away."],
      ["Luxury tone", "Thank you for your patience. I am truly sorry for the wait and will personally review your order to provide a clear update."],
      ["Short", "I am sorry for the delay. I will check your order now and send you an update as quickly as possible."],
      ["French", "Je suis désolé pour ce délai et je comprends votre frustration. Je vais vérifier votre commande et revenir vers vous rapidement."],
    ],
  },
  {
    type: "excel",
    ...
    bullets: ["Clean data", "Find duplicates", "Explain blank cells", "Create formulas", "Detect anomalies", "Summarize KPIs"],
  },
  {
    type: "gpt",
    title: "Late Supplier: complex rules, simple execution",
    subtitle: "A GPT is ideal when the process is stable and repeatable.",
    bullets: [
    ...
    text: "This is not coding. This is business expertise translated into instructions that AI can execute repeatedly.",
  },
  {
    type: "exercise",
    title: "Exercise: improve one real Excel file",
    subtitle: "Ask AI to go one step further.",
    prompt: "Analyze this file. Identify duplicates, blank cells and inconsistent values. Explain why they matter. Suggest formulas or concatenations to improve it. Then propose 3 automation opportunities.",
  },
  {
    type: "agentbuild",
    title: "Design your own AI Agent",
    subtitle: "Even without paid access, everyone can design the workflow.",
    bullets: ["Trigger: when should it run?", "Sources: what should it check?", "Task: what should it do?", "Output: what should it send?", "Audience: who receives it?"],
  },
  {
    type: "hub",
    title: "Live Hub build",
    subtitle: "Give me a department document. We turn it into a Hub category in 5 minutes.",
    bullets: ["Summarize the document", "Create a menu category", "Structure the page", "Add actions and FAQs", "Publish reusable knowledge"],
  },
  {
    type: "next",
    title: "What should you try next?",
    subtitle: "Pick one format and one task.",
    cards: [
      ["Chat", "Rewrite this email professionally."],
      ["Project", "Summarize these files and list next actions."],
      ["GPT", "Analyze this report every week with the same rules."],
      ["Agent", "Monitor this topic daily and email me a digest."],
      ["Hub", "Turn this document into a training page."],
    ],
  },
];
  ...
  return (
    <main style={styles.page}>
      <header style={styles.header}>
        <div>
          <div style={styles.kicker}>Tenengroup AI Training</div>
      ...
      <section style={styles.slide}>
        ...
        {s.type === "rewrite" && <Rewrite s={s} version={version} setVersion={setVersion} />}
        {s.type === "gpt" && <Gpt s={s} />}
...
function Intro({ s }) {
  return (
    <div style={styles.twoCols}>
      <div style={styles.highlight}>{s.text}</div>
      <Bullets s={s} />
    </div>
  );
}
    ...
    <div style={styles.white}>
      ...
      <div style={styles.grid2}>
        {s.options.map((o, idx) => (
          <div key={o} style={{
            ...styles.option,
            background: show && idx === s.answer ? "#bbf7d0" : "#f8fafc",
            borderColor: show && idx === s.answer ? "#16a34a" : "#e2e8f0"
          }}>
            {o}
          </div>
        ))}
      </div>
      <button style={styles.darkBtn} onClick={() => setShow(!show)}>
        {show ? "Hide answer" : "Reveal answer"}
      </button>
      {show && <div style={styles.answer}>{s.explanation}</div>}
    </div>
  );
}
...
function Case({ s }) {
  return (
    <>
      <div style={styles.twoCols}>
        <div style={{ ...styles.white, background: "#fef3c7" }}>
          <div style={styles.kickerDark}>{s.leftTitle}</div>
          <p style={styles.bigText}>{s.left}</p>
        </div>
        <div style={styles.white}>
          <div style={styles.kickerDark}>{s.rightTitle}</div>
...
function Rewrite({ s, version, setVersion }) {
  return (
    <div style={styles.twoCols}>
      <div style={{ ...styles.white, background: "#fee2e2" }}>
        <div style={styles.kickerDark}>Original</div>
      ...
      <div style={styles.white}>
        ...
        <p style={styles.bigText}>“{s.versions[version][1]}”</p>
      </div>
    </div>
  );
}
...
const styles = {
  ...
  cardText: { fontSize: 24, fontWeight: 900 },
